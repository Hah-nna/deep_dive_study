...
플래그는 옵션이므로 선택적으로 사용함. 플래그를 사용하지 않은 경우 문자열 내 검색 매칭 대상이 1개 이상이더라도 첫 번째 매칭한 대상만을 검색하고 종료함
...
이것을 간단하게 표현하면 아래와 같음. `\d`는 숫자를 의미함. `\D`는 `\d`와 반대로 동작함
...
exec 메소드는 g플래그를 지정하여도 첫 번째 매칭 결과만을 반환함
